...
console.log('--', text.at(-3)); // includes randa ar ieskoma reiksme yra string eiluteje.Duoda true arba false. 
...
console.log('Bepasikulversciaudamasis'.includes('kul')); // includes iesko visama duotame tekste.

console.log('---------startsWith------'); // startsWith ziuri ar nuo pradzios ieskoma raide yra. pvz P./ duoda true arba false 

console.log('Pomirodas'.startsWith('a'));
console.log('Pomirodas'.startsWith('P'));
console.log('Pomirodas'.startsWith('Po'));
console.log('Pomirodas'.startsWith('Pom')); // endsWith  ar pabaiga baigiasi. duoda true arba false. 
...
console.log('-----indexOF-----'); // indexOf kiek raidziu zodyje ir raides pozicija, jeigu yra daugiau tokiu paciu, rodo pirmaos raides index.
...
console.log('------trim--------'); //trim() pasalina nereikalingus tarpus tarp zodziu. 
// Vartotojo įvesties valymas

var vartotojoIvestis = '   john.doe@example.com   ';
var valytasElPastas = vartotojoIvestis.trim(); // Palyginimams
...
var symbol = "labas";
var megaSymbol = symbol.repeat(10); // repeatas floor padaro automatiskai. 

console.log(symbol);
console.log(megaSymbol);

function stringRepeat(text, count) {
  /// pakrtoja sita teksta 3 kartus.
  var result = '';

  for (var i = 0; i < Math.floor(count); i++) {
    // darome 3 kartus cikla. kuris lygi tuscias plius text+text+text
    result += text;
  }
...
console.log('--------replace-------'); // replace () pakeiciame vietoje kazko i norima kazka kita 

console.log('a');
console.log('vasara'.replace('a', '-')); /// (pirmoje rasosi ko ieskai, antram raso kuo keiciam)

var summer = 'vasara'.replace('a', '-');
console.log(summer);
var summer2 = 'vasara'.replace('a', '-').replace('a', '-').replace('a', '-'); /// galima chainint eilutes funkcijos. 

console.log(summer2);
var cloud = 'debeselis'.replace('u', '***').replace('e', '*'.repeat(5)); /// galima ideti kita metoda. pvz repeat. 

console.log(cloud);
var template = 'VARDAS ejo i parduotuve pirkti DAIKTAS.'; // kai norime sakinius replacint

var name = 'Jonas';
var item = 'Pomidoras';
var shop = template.replace('VARDAS', name).replace('DAIKTAS', item);
console.log(shop);
console.log('------replaceAll()------'); /// replaceall() pakeicia visas norimas naujomis reiksmemis. 

console.log('vasara');
console.log('vasara'.replaceAll('a', '-'));
var template3 = 'VARDAS saunuolis.  VARDAS grazuolis. VARDAS smalsuolis. VARDAS murzius.';
var multiName = template3.replaceAll('VARDAS', 'Petras');
console.log(multiName);
console.log('ananasas'.replace('a', ''));
console.log('ananasas'.replaceAll('a', ''));
console.log('ananasas'.replaceAll('na', '')); // Labas rytas, Lietuva

var hi = 'Labas rytas, Lietuva!';
var hiNormalSize = hi.replace(' ', '').length; //// pasalinam tarpus ir paskaiciuojam raidziu kieki. 

console.log(hiNormalSize);
console.log('--------toUpperCase-----'); //// toUpperCase() PADAROM I DIDESNES RAIDES TEKSTA. 

console.log('labas'.toUpperCase());
console.log('Labas'.toUpperCase()); /// toLowerCase() padaro teksta mazosiomis raidemis. 
...
console.log('-----slice-------'); /// slice() nupjauna norima kieki zodzio.pirmas skaicius nuo kur antras iki kur. 
...
console.log("\"".concat('pomidoras'.trim(), "\""));
console.log("\"".concat('pomidoras'.trim(), "\""));
console.log("\"".concat('pomidoras'.trim(), "\""));
console.log("\"".concat('labas rytas'.trim(), "\"")); /// trimEnd() nuvalo tik gala. 
/// trimStart() nuvalo tik prieki. 
// Vardas --> Vardemnis Pavardenis
// Pavarde--> Pavardenis

console.log('------split-------'); // split(); perskiria zodzius pagal norima skirtuka, pvz -_ skirtukas.Grazina zodi kaip stringa array tipe. 

var t1 = 'zodis';
var t1Parts = t1.split(); // grazina stringu['zodis'];

var t1Parts2 = t1.split('o'); // o ispjauna ir padaro du zodzius. 

var t1Parts3 = t1.split('d'); // d ispjauna ir palieka zo is 

console.log(t1Parts);
console.log(t1Parts2);
console.log(t1Parts3);
var t2 = 'kultuvatorius';
var t2Parts = t2.split('t');
console.log(t2Parts);
var t3 = 'Labas rytas Lietuva';
var t3Parts = t3.split(' ');
console.log(t3Parts);
...
var t4 = 'Labas. Ka tu. Ka tu veiki';
var sentenceCount = t4.split('.').length - 1;
...
var s1 = String.fromCharCode(65); // is kodo suzinota simbolis abeceleje. 

console.log(s1);
var s2 = String.fromCharCode(4845); // is kodo suzinota simbolis abeceleje. Visi simboliai yra.

console.log(s2);
var s3 = String.fromCharCode(100001); // is kodo suzinota simbolis abeceleje. Visi simboliai yra.
